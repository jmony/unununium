...
****************************************************************************/
static int PMAPI SuspendCallbackFunc(
        int flags)
{
...
//            if (doTest == doOpenGLTest && (mi.XResolution > 2048 || mi.YResolution > 2048 || mi.MaxBuffers < 2))
//                continue;
...
static void OpenGLTests(
    GC_devCtx *gc)
{
  GC_printf(gc,"OpenGL tests are disabled...");
  EVT_getch();
#if 0
    int         choice;
    GA_glCtx    *gl = NULL;
...
static PyObject *run( PyObject *self, PyObject *args )
{
    ...
    if ((dc = GA_loadDriver(0,false)) == NULL)
        PM_fatalError("Unable to load driver!");
...
static PyMethodDef gatestMethods[] = {
...
void initgatest() {
  Py_InitModule( "gatest", gatestMethods );
}
