...
static void* _alloc_libc_malloc(size_t size) {
  ...
  asm(
      ...
      "xorl %0, %0\n"\
      "continue%=:\n"\
      : "=a" (ptr) : "a" (size) : "ebx","ecx","edx","cc" );

  if( ptr == NULL ) goto err_out;
  return ptr;
...
void* __libc_realloc(void* ptr, size_t _size) {
  ...
  asm(
      ...
      : "=a" (ptr), "=b" (trash) : "a" (_size), "b" (ptr) : "ecx","edx","cc" );
